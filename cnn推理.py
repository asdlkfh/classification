...
class TextCNN(nn.Module):
    def __init__(self,parameter):
        super().__init__()
        filter_size = (3,4,5)
        channels_size = parameter['kernel_size']
        output_size = parameter['output_size']
        dropout = parameter['dropout']
        word_size = parameter['word_size']
        embedding_dim = parameter['embedding_dim']
        self.embedding = nn.Embedding(word_size,embedding_dim,padding_idx = 0)
        self.convs = nn.ModuleList([nn.Conv2d(1,channels_size,(k,embedding_dim)) for k in filter_size])
        self.dropout = nn.Dropout(dropout)
        self.fc = nn.Linear(channels_size*3,output_size)
    def forward(self,x):
        x = self.embedding(x)
        x = x.unsqueeze(1)
...
model = TextCNN(parameter).to(device)
...
count_pandas = pd.DataFrame(count_dict.values(), index=label_name,columns=['pred_pos','real_pos','true_pos'])
